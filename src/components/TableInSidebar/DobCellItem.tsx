...
interface DobCellItemProps {
    ...
    isReadOnly: boolean;
    flatData: any;
    onFocusCell: (documentId: string, datapointId: string, activeDatapoint: any) => void;
    onKeyDownCell: (e: React.KeyboardEvent, document_id: string, datapoint_id: string, index: number) => void;
    onMouseDownCell: (datapointId: string) => void;
...
const DobCellItem: React.FC<DobCellItemProps> = ({
    ...
    useEffect(() => {
        if (inputCellRefs[index]?.current && datapointRef.current) {
            // @ts-ignore
            datapointRef.current = inputCellRefs[index].current;
        }
    }, [inputCellRefs, index]);
...
export default React.memo(DobCellItem)
