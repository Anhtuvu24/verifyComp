...
// Styles
import { Datapoint, Title, SelectStyles } from "../index.styles";
...
interface GenderDatapointItemProps {
    datapoint: any;
    activeFieldData: any;
    fieldDataValues: any;
    inputRefs: any;
...
const GenderDatapointItem: React.FC<GenderDatapointItemProps> = ({
    ...
    const array = focusDatapoints && useGetIndex(focusDatapoints, field_data_id, 'not_validated');
    const [index] = array;
    const datapointRef = inputRefs[index];

    // const syncStatus = useSelector((state: any) => state.docbase.entities.datapoint.all[datapoint_id]?.status); // status cua datapoint
    // const fieldDataValue = useSelector((state: any) => state.docbase.entities.field_data.values[field_data_id]); // value cua field_data
    const fieldDataValue = fieldDataValues ? fieldDataValues[field_data_id]?.value : ''; // value cua field_data
    ...
    const onChange = (value: unknown) => {
        ...
        debounced(field_data_id, stringValue);
    };
        ...
        <Datapoint name={`datapoint-${field_data_id}`}>
            ...
            <Form form={form} component={false}>
                <Form.Item name={field_data_id} style={{ flex: 1, marginBottom: 0 }}>
                    ...
                    >
                        <Option value={'Nam'} style={{ fontSize: 13 }}>
                            Nam
                        </Option>
                        <Option value={'Nữ'} style={{ fontSize: 13 }}>
                            Nữ
                        </Option>
                        <Option value={'Khác'} style={{ fontSize: 13 }}>
                            Khác
                        </Option>
                    </SelectStyles>
                </Form.Item>
...
export default React.memo(GenderDatapointItem);
